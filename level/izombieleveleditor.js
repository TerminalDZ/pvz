(function () {
	/*
		ChoseMode: Level Selection Screen
		NGrass: Night Grass
		NPool: Night Pool
		DReversal: Water Path Reversal
	*/
	var IZMode = oS.NowLevel ?? "ChoseMode"; // Read current level mode
	var Change_Level = function (ModeName) {
		(oS.NowLevel = ModeName), SelectModal(oS.Lvl), (oS.NowLevel = ModeName);
	}; // Reload this level in a specific mode
	var $FJ = function (a, b) {
		// Merge arrays
		var ret = {};
		for (var i in a) {
	...
	var oSys = {
		...
		LvlClearFunc() {
			...
			delete oS.NowLevel; // Clear level stage data
		},
		ArP: { ArC: [1, 4], ArR: [1, 5] },
		LoadAccess(a) {
			!oS.LvlVar ? (oS.LvlVar = { ScrollScreen: oS.ScrollScreen }) : (oS.LvlVar.ScrollScreen = oS.ScrollScreen); // Level data
		...
		StartGame() {
			oP.Monitor({
				ar: [],
				f() {
					var a = NewEle("DivTeach", "div", "line-height:40px;font-size:14px;top:380px", 0, EDAll); // Select formation columns
					var b = function (c) {
						ClearChild($("DivTeach")), ImmediatelyCool(); // Unfreeze all plants
						SetVisible($("tdShovel"), $("dFlagMeter")); // Show shovel
						NewImg("iStripe", "images/interface/Stripe.png", "left:" + (GetX1X2(c)[0] - 11) + "px;top:65px", EDAll); // Generate line
						NewEle(
							"btnClickSave",
							"button",
							"position:absolute;left:750px;top:250px;height:50px;width:100px;font-family:Arial;font-size:18px;z-index:100",
							{
								// Save button
								...
								onclick() {
									...
									var j = {
										// Plant data
										default: "01", // Default
										oPeashooter: "01", // Normal plants 1 ~ 40
										oSunFlower: "02",
										oCherryBomb: "03",
										oWallNut: "04",
										oPotatoMine: "05",
										...
										oCoffeeBean: "36",
										oGarlic: "37",
										oGatlingPea: "41", // Purple cards: 41 ~ 48
										oTwinSunflower: "42",
										oGloomShroom: "43",
										oSpikerock: "47",
										oBrains: "51", // Other plants: 50 ~ 70
										oLawnCleaner: "52",
										oPoolCleaner: "53",
										oNutBowling: "54",
										oHugeNutBowling: "55",
									...
									} // Generate plant data, using reverse order
									...
									if (
										...
										($("btnClickSave").innerHTML = "Saving.."), ($("btnClickSave").disabled = "disabled"); // Button style
										// ajax("asp/ImZombieCreateGame.asp", "post", "mapkind=" + oS.MapKind + "&SNum=" + f + "&T=" + escape(l) + "&C=" + escape(m), function(c){eval(c)}); // Send request
						...
						oS.ArP.ArC = [1, c]; // Define planting range
						oS.ArP.ArR = [1, oS.R]; // Row
					};
					innerText(
						NewEle(
							"spanT",
	...
	// Start different levels based on different modes
				...
				$FJ(oSys, {
					...
					!oS.LvlVar ? (oS.LvlVar = { ScrollScreen: oS.ScrollScreen }) : (oS.LvlVar.ScrollScreen = oS.ScrollScreen); // Level data
						...
						NewEle(
							"dChoseTitle",
							"div",
							"position:relative;text-align:center;line-height:88px;height:88px;width:100%;font-size:30px;font-weight:bold;font-family:Arial;color:#fff",
						...
						NewEle(
							"dGrassTXT",
							"div",
							"text-align:center;line-height:60px;font-size:30px;font-weight:bold;font-family:Arial;color:#fff;position:relative;top:15px;",
							{
						...
						SetVisible($("dMenu")); // Show menu button
					},
					LvlClearFunc() {
						oS.ScrollScreen = oS.LvlVar.ScrollScreen;
						delete oS.LvlVar.ScrollScreen;
